...
public:
    ...
    void open_disk_rw()
    {
        if (!disk.is_open())
        {
            ...
            if (!disk)
                throw runtime_error("Cannot open disk image.");
        }
    }
    ...
    // REVISION 4: File Info Command
    void cmd_fileinfo(const string &fname)
    {
        size_t idx = find_file_index(fname, true);
        if (idx == SIZE_MAX)
        {
            cout << "File not found.\n";
            return;
        ...
        cout << "Size: " << fe.size << " bytes\n";
        cout << "Blocks Used: " << fe.block_count << "\n";
        cout << "Deleted: " << (fe.deleted ? "Yes" : "No") << "\n";
        cout << "Block List: ";
        for (size_t i = 0; i < fe.block_count; i++)
        {
            cout << fe.blocks[i] << (i + 1 < fe.block_count ? ", " : "");
        }
        cout << "\n----------------------\n";
    }
    ...
    void optimize_disk()
    {
        ...
        for (auto &fe : file_table)
        {
            ...
            for (size_t i = 0; i < fe.block_count; ++i)
            {
                while (next_free < NUM_BLOCKS && new_bitmap[next_free])
                    next_free++;
    ...
    void repl()
    {
        ...
        while (true)
        {
            ...
            else if (cmd == "delete")
            {
                ...
                cmd_list();
            else if (cmd == "showbitmap")
                cmd_showbitmap();
            else if (cmd == "recover")
                recover_from_journal();
            else if (cmd == "optimize")
                optimize_disk();
            // REVISION 4: fileinfo command
