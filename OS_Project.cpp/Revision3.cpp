...
public:
    ...
    // ===== Revision 3: Storage usage summary =====
    void cmd_usage() {
        size_t used = count(bitmap.begin(), bitmap.end(), 1);
        size_t free_blocks = NUM_BLOCKS - used;

        double used_percent = (double)used / NUM_BLOCKS * 100.0;
        double free_percent = (double)free_blocks / NUM_BLOCKS * 100.0;

        cout << "------ Storage Usage ------\n";
        cout << "Total Blocks : " << NUM_BLOCKS << "\n";
        cout << "Used Blocks  : " << used << " (" << used_percent << "%)\n";
        cout << "Free Blocks  : " << free_blocks << " (" << free_percent << "%)\n";
        cout << "----------------------------\n";
    }
    ...
    void repl() {
        ...
        while (true) {
            ...
            ss >> cmd;
            if (cmd == "help") {
                cout << "init, create <name>, write <name> <text>, read <name>, delete <name>, list, showbitmap, recover, optimize, usage, exit\n";
            } else if (cmd == "init") {
                init_disk(true);
            } else if (cmd == "create") {
            ...
            else if (cmd == "usage") cmd_usage();  // Revision 3
