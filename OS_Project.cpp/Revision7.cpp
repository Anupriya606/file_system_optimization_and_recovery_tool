...
// REVISION 7
    ...
    void serialize(ostream &os) const {
        ...
        os.write((char*)&created_at, sizeof(created_at)); // REVISION 7
    }
    ...
    void deserialize(istream &is) {
        ...
        is.read((char*)&created_at, sizeof(created_at)); // REVISION 7
    }
...
// REVISION 7
        file_table[idx].created_at = time(nullptr);
            ...
            if (rec.type == J_CREATE) {
                if (idx == SIZE_MAX) {
                    ...
                    if (idx != SIZE_MAX) {
                        ...
                        file_table[idx].deleted = false;
                        file_table[idx].block_count = 0;
                        file_table[idx].created_at = time(nullptr); // REVISION 7
                    }
                }
    ...
    // REVISION 7
    void showCreated(const string &fname) {
        size_t idx = find_file_index(fname, true);
        if (idx == SIZE_MAX) {
            cout << "File not found.\n";
            return;
        }
        cout << "Created At: " << ctime(&file_table[idx].created_at);
    }
    ...
    void repl() {
        ...
        while (true) {
            ...
            ss >> cmd;
            if (cmd == "help") {
                cout << "init, create <name>, write <name> <text>, read <name>, delete <name>, list, showbitmap, recover, optimize, created, exit\n";
            } else if (cmd == "init") {
                init_disk(true);
            } else if (cmd == "create") {
