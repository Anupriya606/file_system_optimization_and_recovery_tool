...
public:
    ...
    // ------------------- REVISION 1: File size limit check -------------------
    void cmd_write(const string &fname, const string &data) {
        ...
        // Revision 1: Check maximum allowed file size
        if (needed_blocks > MAX_BLOCKS_PER_FILE) {
            cout << "Error: File too large. Maximum allowed size is "
                 << (MAX_BLOCKS_PER_FILE * BLOCK_SIZE) << " bytes.\n";
            return;
        }
